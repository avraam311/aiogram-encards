...
from database.requests import orm_add_user, orm_add_to_words_category, orm_get_user_words_categories
...
async def command_start(message: Message, session: AsyncSession) -> None:
    ...
    first_message = await orm_get_user_words_categories(session, user_id=message.from_user.id)
    if first_message == 0:
        await orm_add_to_words_category(session, user_id=message.from_user.id)
        await message.answer("Пользователь добавлен в базу данных с каталогом слов \"1\"")
...
async def user_menu(callback: CallbackQuery, callback_data: MenuCB, session: AsyncSession):
    if callback_data.menu_name == "add_to_words_category":
        ...
        words_categories_limit = await orm_get_user_words_categories(session, user_id=callback.from_user.id)
        if words_categories_limit == 16:
            await callback.answer(f'Максимальное количество каталогов слов: 15')
            return
